...
public class BinaryReaderEx : BinaryReader
{
	...
	public bool IsArma3Format { get; set; }
	...
	public T[] ReadArray<T>(Func<BinaryReaderEx, T> readElement)
	{
		...
		if (count < 0 || count > 1000000)
			throw new InvalidDataException($"ReadArray: invalid count {count}");
		return ReadArrayBase(readElement, count);
	}
	...
	public T[] ReadArray<T>(int count) where T : IDeserializable, new()
	{
		return ReadArrayBase((BinaryReaderEx i) => i.ReadObject<T>(), count);
	}
	...
	public T[] ReadCompressedArray<T>(Func<BinaryReaderEx, T> readElement, int elemSize)
	{
		...
		if (num < 0 || num > 1000000)
			throw new InvalidDataException($"ReadCompressedArray: invalid count {num}");
	...
	public T[] ReadCondensedArray<T>(Func<BinaryReaderEx, T> readElement, int sizeOfT)
	{
		int num = ReadInt32();
		if (num < 0 || num > 1000000)
			throw new InvalidDataException($"ReadCondensedArray: invalid count {num}");
		T[] array = new T[num];
		if (ReadBoolean())
		{
